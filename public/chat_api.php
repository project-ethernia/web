...
if (!$ticket || (!$is_admin && $ticket['user_id'] != $current_user_id)) {
...
function formatMessageData($m, $current_user_id, $is_admin_request) {
    $isSystem = (strpos($m['message'], '[SYSTEM]') === 0);
    $cleanMessage = $isSystem ? trim(substr($m['message'], 8)) : $m['message'];
    
    $isMine = false;
    if (!$isSystem) {
        if ($is_admin_request && $m['is_admin'] == 1 && $m['sender_id'] == $current_user_id) $isMine = true;
        if (!$is_admin_request && $m['is_admin'] == 0 && $m['sender_id'] == $current_user_id) $isMine = true;
    }
    ...
    return [
        ...
        'is_system' => $isSystem,
        'is_mine' => $isMine,
        'is_admin' => (int)$m['is_admin'] === 1,
        'message' => htmlspecialchars($cleanMessage, ENT_QUOTES, 'UTF-8'),
...
if ($action === 'sync') {
    ...
    $msgStmt = $pdo->prepare("
        SELECT tm.*, a.username as admin_username, u.username as user_username 
        FROM ticket_messages tm 
        LEFT JOIN admins a ON tm.sender_id = a.id AND tm.is_admin = 1
    ...
    foreach ($messagesRaw as $m) {
        $messages[] = formatMessageData($m, $current_user_id, $is_admin);
        $new_last_id = $m['id'];
    }
    ...
    $other_typing = ($other_typing_at && strtotime($other_typing_at) >= time() - 3);
...
if ($action === 'send') {
    ...
    if (isset($_SESSION[$session_key]) && ($now - $_SESSION[$session_key]) < 2) {
        echo json_encode(['success' => false, 'error' => 'cooldown']);
        exit;
    }
    ...
    $stmt->execute([$ticket_id, $current_user_id, $message, $attachment, $is_admin_flag]);
